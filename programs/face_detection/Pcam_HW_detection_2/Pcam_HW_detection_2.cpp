...
void display(const Mat& frame,const Mat& Image);
...
void getdata(Mat& frame)
{
	...
	str.clear();
	str="result "+to_string(result_size)+" faces:" +to_string(nbr);
...
void display(const Mat& frame,const Mat& Image)
{
	...
	frame_size = temp.size();
	for (int y = 0; y < frame_size.height ; y++) 
	{
		ofs.seekp((720+y)*framebuffer_width*3);
