...
int main( int argc, const char** argv )
{
	...
	if ( argc != 2 )
    {
		...
		cout<<"CV version:"<<CV_VERSION_EPOCH<<"."<<CV_MAJOR_VERSION<<"."<<CV_MINOR_VERSION<<endl;
	#else
		cout<<"CV version:"<<CV_MAJOR_VERSION<<"."<<CV_MINOR_VERSION<<endl;
	#endif
	
    Mat image;
	image = imread( argv[1], 1 );
	 if ( !image.data )	//check for image
    {
        cout<<"No image data \n";
        return -1;
    }
...
void detectAndDisplay( Mat frame )
{
	...
	for (int y = 0; y < 240 ; y++) 
	{
		...
		ofs.write(reinterpret_cast<char*>(frame.ptr(y)),320*3);
	}
	//-------------------------------------
