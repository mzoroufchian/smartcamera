...
int check_face()
{
	...
	if(!persones.data)
	{
		char in_add;
		Mat temp=face;
		putText(temp, "unkown ", Point(0,90), FONT_HERSHEY_PLAIN, 1.0, Scalar(0,0,255), 2,false);
		display_face(temp,0);
		cout<<"unkown person, want to add:[y/n]";
		cin>>in_add;
		if(in_add=='y')
		{
			if(max_names>ncount)
			{
				...
				cout<<"maximum reached!! inpossible to add"<<endl;
		}
	}
